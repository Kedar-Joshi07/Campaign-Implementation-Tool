from __future__ import annotations

from pathlib import Path

import pytest

from app.database.connection import get_connection
from app.database.schema import initialize_database
from app.repositories.audience_rank_repository import (
    AudienceRankRepository,
    AudienceRankValidationError,
)


@pytest.fixture
def database_path(tmp_path: Path) -> Path:
    path = tmp_path / "audience-rank-repository.db"
    initialize_database(path)
    return path


def _insert_completed_analysis(database_path: Path) -> int:
    with get_connection(database_path, write=True) as connection:
        cursor = connection.execute(
            """
            INSERT INTO historical_analysis_runs (
                analysis_name, created_at, completed_at, status,
                conversion_definition, filters_json, results_json,
                observation_count, selected_customer_count,
                positive_customer_count, unlabeled_customer_count,
                positive_customer_rate
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                "Audience rank fixture",
                "2026-09-01T00:00:00Z",
                "2026-09-01T00:00:03Z",
                "COMPLETED",
                "ATTRIBUTED_PURCHASE",
                "{}",
                "{}",
                100,
                20,
                5,
                15,
                0.25,
            ),
        )
        return int(cursor.lastrowid)


def _insert_model_run(database_path: Path, analysis_run_id: int) -> int:
    with get_connection(database_path, write=True) as connection:
        cursor = connection.execute(
            """
            INSERT INTO model_runs (
                analysis_run_id,
                model_name,
                created_at,
                status,
                random_seed,
                validation_fraction
            ) VALUES (?, ?, ?, ?, ?, ?)
            """,
            (
                analysis_run_id,
                "Audience rank model",
                "2026-09-01T00:00:05Z",
                "RUNNING",
                42,
                0.2,
            ),
        )
        return int(cursor.lastrowid)


def _insert_scoring_job(database_path: Path, model_run_id: int) -> int:
    with get_connection(database_path, write=True) as connection:
        cursor = connection.execute(
            """
            INSERT INTO jobs (
                job_type,
                status,
                progress_percent,
                stage,
                analysis_run_id,
                model_run_id,
                created_at,
                started_at,
                request_json,
                result_json,
                error_message
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                "PROSPECT_SCORING",
                "RUNNING",
                1,
                "VALIDATING_MODEL",
                None,
                model_run_id,
                "2026-09-01T00:00:10Z",
                "2026-09-01T00:00:10Z",
                "{}",
                None,
                None,
            ),
        )
        return int(cursor.lastrowid)


def _insert_scoring_run(database_path: Path, job_id: int, model_run_id: int) -> int:
    with get_connection(database_path, write=True) as connection:
        cursor = connection.execute(
            """
            INSERT INTO scoring_runs (
                job_id,
                model_run_id,
                created_at,
                completed_at,
                status,
                demographic_snapshot_count,
                demographic_min_person_id,
                demographic_max_person_id,
                scored_person_count,
                chunk_size,
                selected_candidate,
                model_role_policy_version,
                feature_contract_version,
                feature_contract_sha256,
                artifact_sha256,
                score_min,
                score_max,
                score_mean,
                score_summary_json,
                error_message
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                job_id,
                model_run_id,
                "2026-09-01T00:00:11Z",
                "2026-09-01T00:00:20Z",
                "COMPLETED",
                100,
                "PER_001",
                "PER_100",
                100,
                1000,
                "BAGGING_PU",
                "2",
                "1",
                "a" * 64,
                "b" * 64,
                0.1,
                0.9,
                0.5,
                "{}",
                None,
            ),
        )
        return int(cursor.lastrowid)


def _build_boundaries() -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    total_population = 100
    for bucket in range(1, 101):
        rows.append(
            {
                "percentile_bucket": bucket,
                "boundary_rank": bucket,
                "boundary_score": max(0.0, 1.0 - (bucket / 100.0)),
                "boundary_person_id": f"PER_{bucket:03d}",
                "total_population": total_population,
            }
        )
    return rows


def test_replace_and_fetch_boundaries(database_path: Path) -> None:
    analysis_run_id = _insert_completed_analysis(database_path)
    model_run_id = _insert_model_run(database_path, analysis_run_id)
    job_id = _insert_scoring_job(database_path, model_run_id)
    scoring_run_id = _insert_scoring_run(database_path, job_id, model_run_id)

    repository = AudienceRankRepository(database_path)
    repository.replace_boundaries(
        scoring_run_id=scoring_run_id,
        rank_contract_version="1",
        created_at="2026-09-01T00:20:00Z",
        boundaries=_build_boundaries(),
    )

    fetched = repository.fetch_boundaries(scoring_run_id)
    assert len(fetched) == 100
    assert fetched[0]["percentile_bucket"] == 1
    assert fetched[-1]["percentile_bucket"] == 100
    assert fetched[-1]["boundary_rank"] == fetched[-1]["total_population"]


def test_replace_boundaries_validates_cardinality_and_bucket_coverage(database_path: Path) -> None:
    analysis_run_id = _insert_completed_analysis(database_path)
    model_run_id = _insert_model_run(database_path, analysis_run_id)
    job_id = _insert_scoring_job(database_path, model_run_id)
    scoring_run_id = _insert_scoring_run(database_path, job_id, model_run_id)

    repository = AudienceRankRepository(database_path)
    boundaries = _build_boundaries()[:-1]
    with pytest.raises(AudienceRankValidationError, match="Exactly 100"):
        repository.replace_boundaries(
            scoring_run_id=scoring_run_id,
            rank_contract_version="1",
            created_at="2026-09-01T00:20:00Z",
            boundaries=boundaries,
        )

    boundaries = _build_boundaries()
    boundaries[99]["percentile_bucket"] = 99
    with pytest.raises(AudienceRankValidationError, match="unique|cover all integers"):
        repository.replace_boundaries(
            scoring_run_id=scoring_run_id,
            rank_contract_version="1",
            created_at="2026-09-01T00:20:00Z",
            boundaries=boundaries,
        )


def test_replace_boundaries_validates_percentile_100_rank(database_path: Path) -> None:
    analysis_run_id = _insert_completed_analysis(database_path)
    model_run_id = _insert_model_run(database_path, analysis_run_id)
    job_id = _insert_scoring_job(database_path, model_run_id)
    scoring_run_id = _insert_scoring_run(database_path, job_id, model_run_id)

    repository = AudienceRankRepository(database_path)
    boundaries = _build_boundaries()
    boundaries[-1]["boundary_rank"] = 99

    with pytest.raises(AudienceRankValidationError, match="100th percentile"):
        repository.replace_boundaries(
            scoring_run_id=scoring_run_id,
            rank_contract_version="1",
            created_at="2026-09-01T00:20:00Z",
            boundaries=boundaries,
        )
